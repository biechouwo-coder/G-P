...
def run_fe_regression(df, cluster_var='city_name'):
    ...
    # 控制变量（只有 ln_pgdp 和 ln_pop_density）
    X_controls = df[['ln_pgdp', 'ln_pop_density']].values
    ...
    print(f"  核心变量: DID")
    print(f"  控制变量: ln_pgdp, ln_pop_density（仅2个控制变量）")
...
# 提取关键变量的结果（前4个变量：常数项、DID、2个控制变量）
var_names = ['常数项', 'DID', 'ln_pgdp', 'ln_pop_density']
n_key_vars = len(var_names)

print(f"\n{'='*80}")
print(f"DID回归结果（仅含ln_pgdp和ln_pop_density控制变量）")
print(f"{'='*80}")
...
# 保存结果到Excel
output_file = '人均GDP+人口集聚程度+产业高级化+人均道路面积+金融发展水平/DID回归结果_仅基础控制变量.xlsx'
...
with pd.ExcelWriter(output_file, engine='openpyxl') as writer:
    ...
    # 模型摘要
    summary_data = {
        '指标': ['样本量', '城市聚类数', 'R-squared', '固定效应', '控制变量数'],
        '数值': [results['n_obs'], results['n_clusters'], f"{results['r_squared']:.4f}",
                 f"城市FE ({len(city_fe)}个) + 年份FE ({len(year_fe)}个)", '2个（ln_pgdp, ln_pop_density）']
    }
...
print(f"模型对比（不同控制变量）:")
print(f"{'='*80}")
print(f"{'模型':<40} {'样本量':>8} {'DID系数':>12} {'p值':>10}")
print(f"{'-'*80}")
print(f"{'仅基础控制变量（ln_pgdp, ln_pop_density）':<40} {results['n_obs']:>8} {did_coef:>12.6f} {did_p:>10.4f}")
print(f"{'完整控制变量（+产业高级化+道路面积+金融发展）':<40} {3623:>8} {0.017066:>12.6f} {0.5539:>10.4f}")
print(f"{'-'*80}")
