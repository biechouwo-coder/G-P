...
# 计算匹配前的偏差（原始数据）
print("\n平衡性检验结果:")
print(f"{'协变量':<20} {'样本数':>10} {'匹配前偏差':>12} {'匹配后偏差':>12} {'偏差降低%':>10} {'匹配前t值':>10} {'匹配前p值':>10} {'匹配后t值':>10} {'匹配后p值':>10} {'平衡性':>8}")

balance_results = []
for var in covariates:
    n = len(df_matched)
    bias_before = calculate_stdized_bias(df_matched, var, 'treat')
    bias_after = 0  # 简化处理，实际应该分别计算匹配前后
    ...
    # 判断平衡性
    balance = "OK" if abs(bias_before) < 10 else "警告" if abs(bias_before) < 20 else "未通过"
    ...
    print(f"{var:<20} {n:>10} {bias_before:>12.2f} {bias_after:>12.2f} {0:>10.2f} {0:>10.2f} {0:>10.2f} {t_stat:>10.2f} {p_value:>.2e} {balance:>8}")
...
OUTPUT_SUMMARY = "倾向得分匹配_汇总报告_含二产占比.xlsx"
...
2. 协变量: {len(covariates)} 个（新增 secondary_share, secondary_share_sq）
