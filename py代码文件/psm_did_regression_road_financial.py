...
def run_psm_did_regression(df, cluster_var='city_name'):
    """
    运行PSM-DID回归（固定效应模型 + 城市聚类标准误）
    """
    # 准备数据
...
# 读取PSM匹配后数据集
df = pd.read_excel('人均GDP+人口集聚程度+产业高级化+人均道路面积+金融发展水平/PSM_匹配后数据集.xlsx')
print(f"[INFO] PSM匹配后数据集形状: {df.shape}")
print(f"[INFO] 城市数: {df['city_name'].nunique()}")

# 检查样本构成
treat_obs = len(df[df['treat'] == 1])
control_obs = len(df[df['treat'] == 0])
print(f"[INFO] 处理组观测数: {treat_obs}")
print(f"[INFO] 控制组观测数: {control_obs}")

# 运行PSM-DID回归
results, city_fe, year_fe = run_psm_did_regression(df)
...
# 保存结果到Excel
output_file = '人均GDP+人口集聚程度+产业高级化+人均道路面积+金融发展水平/PSM-DID回归结果.xlsx'
...
with pd.ExcelWriter(output_file, engine='openpyxl') as writer:
    ...
    # 模型摘要
    summary_data = {
        '指标': ['样本量', '城市聚类数', 'R-squared', '固定效应', 'PSM匹配成功率', 'PSM卡尺范围'],
        '数值': [
            results['n_obs'],
            results['n_clusters'],
...
# 与未匹配样本对比
print(f"\n{'='*80}")
print(f"PSM-DID vs 未匹配DID对比:")
print(f"{'='*80}")
print(f"{'模型':<25} {'样本量':>10} {'DID系数':>12} {'p值':>10}")
print(f"{'-'*60}")
print(f"{'未匹配DID':<25} {3623:>10} {0.017066:>12.6f} {0.5539:>10.4f}")
print(f"{'PSM-DID':<25} {results['n_obs']:>10} {did_coef:>12.6f} {did_p:>10.4f}")
print(f"{'-'*60}")
