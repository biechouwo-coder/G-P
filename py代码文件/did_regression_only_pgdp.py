...
# 提取关键变量的结果（前3个变量：常数项、DID、ln_pgdp）
var_names = ['常数项', 'DID', 'ln_pgdp']
n_key_vars = len(var_names)

print(f"\n{'='*80}")
print(f"DID回归结果（仅含ln_pgdp控制变量）")
print(f"{'='*80}")
...
# 保存结果到Excel
output_file = '人均GDP+人口集聚程度+产业高级化+人均道路面积+金融发展水平/DID回归结果_仅人均GDP.xlsx'
...
with pd.ExcelWriter(output_file, engine='openpyxl') as writer:
    ...
    summary_data = {
        ...
        '数值': [results['n_obs'], results['n_clusters'], f"{results['r_squared']:.4f}",
                 f"城市FE ({len(city_fe)}个) + 年份FE ({len(year_fe)}个)", '1个（ln_pgdp）']
    }
...
print(f"模型对比（不同控制变量）:")
print(f"{'='*80}")
print(f"{'模型':<40} {'样本量':>8} {'DID系数':>12} {'p值':>10} {'R²':>8}")
print(f"{'-'*80}")
print(f"{'仅人均GDP（ln_pgdp）':<40} {results['n_obs']:>8} {did_coef:>12.6f} {did_p:>10.4f} {results['r_squared']:>8.4f}")
print(f"{'基础控制（ln_pgdp + ln_pop_density）':<40} {3623:>8} {0.011027:>12.6f} {0.7098:>10.4f} {0.9706:>8.4f}")
print(f"{'完整控制（5个变量）':<40} {3623:>8} {0.017066:>12.6f} {0.5539:>10.4f} {0.9712:>8.4f}")
print(f"{'-'*80}")
