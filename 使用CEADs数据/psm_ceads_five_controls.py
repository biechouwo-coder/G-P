...
# 检查缺失值
print("\n各变量缺失值统计:")
vars_to_check = covariates + ['treat']
...
print(f"删除后剩余: {df_clean.shape[0]} 行观测")
print(f"删除后城市数: {df_clean['city_name'].nunique()} 个")
...
if balanced_vars == total_vars:
    print("\n[OK] 所有协变量均通过平衡性检验，匹配质量良好！")
else:
    print(f"\n[WARNING] {total_vars - balanced_vars} 个变量未通过平衡性检验，可能需要调整卡径或增加协变量")
...
if p_value > 0.05:
    ...
    print("  [WARNING] 匹配后倾向得分仍有显著差异，可能需要调整匹配策略")
...
# 保存到Excel
output_file = 'CEADs_PSM_匹配后数据集_五个控制变量.xlsx'
...
# 保存平衡性检验结果
balance_output = 'CEADs_PSM_平衡性检验结果_五个控制变量.xlsx'
...
print(f"  2. 匹配方法: 1:1最近邻匹配，有放回，卡径=0.02")
print(f"  3. 匹配率: {len(matched_pairs)/df[df['treat']==1].shape[0]*100:.1f}%")
print(f"  4. 匹配后样本: {len(df_matched)} 观测, {df_matched['city_name'].nunique()} 城市")
print(f"  5. 平衡性: {balanced_vars}/{total_vars} 变量通过检验")
