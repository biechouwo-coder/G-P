...
print("=" * 80)
print("CEADs数据集描述性统计报告（V2版本 - Bug已修复）")
print("=" * 80)
...
output_file = 'CEADs_描述性统计表_V2.xlsx'
descriptive_df.to_excel(output_file, index=False)
print(f"[OK] 详细统计表已保存至: {output_file}")

# 7. 生成核心发现总结
print("\n" + "=" * 80)
print("核心发现总结（V2版本 vs V1版本）")
print("=" * 80)

print(f"\n1. 数据覆盖情况:")
print(f"   - V1版本: 2,271个观测, 212个城市")
print(f"   - V2版本: {ceads_valid}个观测, {ceads_cities}个城市")
print(f"   - 增加: {ceads_valid - 2271}个观测, {ceads_cities - 212}个城市")
print(f"   - 修复: 吉林市(11观测)、北京市(13观测)、上海市(13观测)")

print(f"\n2. 碳排放强度水平:")
print(f"   - V1均值: 2.7980 吨/万元")
print(f"   - V2均值: {df_ceads['carbon_intensity_ceads_winsor'].mean():.4f} 吨/万元")
print(f"   - 变化: {df_ceads['carbon_intensity_ceads_winsor'].mean() - 2.7980:.4f} 吨/万元")
