...
print(f"\n提取核心列后: {df_ceads_clean.shape[0]} 行观测")
...
def create_match_key(city_name):
    ...
    for suffix in sorted(suffixes, key=len, reverse=True):  # 从长到短匹配
...
df_ceads_clean['match_key'] = df_ceads_clean['city_name_ceads'].apply(create_match_key)

print("\nCEADs城市名示例（前20个）:")
print(df_ceads_clean[['city_name_ceads', 'match_key']].drop_duplicates().head(20))

# 5. 保存清洗后的CEADs数据
ceads_output = 'CEADs_2007-2019_清洗后.xlsx'
...
print(f"列名: {df_gdp_raw.columns.tolist()}")
print(f"\n前5行原始数据:")
print(df_gdp_raw.head())

# 2. 根据列位置提取数据（避免中文编码问题）
# 列结构：0=年份, 1=省份代码, 2=城市代码, 3=城市名, 4=实际GDP, 5=GDP平减指数, 6=名义GDP
# 从列名看：'省份', '地区', '年份', '实际GDP', 'GDP指数', 'GDP平减指数'
df_gdp = df_gdp_raw.iloc[:, [2, 1, 3, 5]].copy()  # 年份, 地区名, 实际GDP, GDP平减指数
...
print(f"\n转换类型后，年份范围: {df_gdp['year'].min()} - {df_gdp['year'].max()}")
...
print(f"\n截断至2007-2019年后: {df_gdp.shape[0]} 行观测")
print(f"年份范围: {df_gdp['year'].min()} - {df_gdp['year'].max()}")

# 5. 构造匹配键
df_gdp['match_key'] = df_gdp['city_name_gdp'].apply(create_match_key)

print("\nGDP城市名示例（前20个）:")
print(df_gdp[['city_name_gdp', 'match_key']].drop_duplicates().head(20))

# 6. 保存清洗后的GDP数据
gdp_output = '实际GDP_2007-2019_清洗后.xlsx'
